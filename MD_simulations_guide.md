...
-  **3-NPT_annealing.** - NPT annealing equilibration using a stronger barostat and thermostat. Here we are going to increase and decrase the temperature around our target value. This help the system to traverse the potential energy surface. Also, some systems like ionic liquids can present metastable phases depending of the temperature rate equilibration. Remember that if you use a temperature that goes too high in relation to the boiling point, it will be hard to your system come back to the liquid state. Think about the temperatures and the cooling/heating rate that you are going to use with caution.
...
- Check all the job.md.N files regarding the queue, ncpus, -ntomp and -np XX gmx_mpi. Remember that **-ntomp X -np MUST BE EQUAL ncpus** this is a critical part and if wrong can create problem to other jobs from other users in the cluster.
